          ...
          <UISelect.Popup className="outline-hidden group origin-[var(--transform-origin)] rounded-lg bg-[canvas] py-1 text-default shadow-lg transition-[transform,scale,opacity] data-ending-style:scale-100 data-ending-style:opacity-100 data-ending-style:transition-none data-starting-style:scale-90 data-starting-style:opacity-0 data-[side=none]:data-starting-style:scale-100 data-[side=none]:data-starting-style:opacity-100 data-[side=none]:data-starting-style:transition-none">
            ...
            <UISelect.List className="relative py-1 scroll-py-6 overflow-y-auto max-h-[var(--available-height)]">
              {options.map(({ label, value }) => (
                ...
                >
                  <UISelect.ItemText className="col-start-2">
                    {label}
                  </UISelect.ItemText>
                </UISelect.Item>
              ))}
            </UISelect.List>
            <UISelect.ScrollDownArrow className="bottom-0 z-[1] flex h-4 w-full cursor-default items-center justify-center rounded-md bg-[canvas] text-center text-xs before:absolute before:left-0 before:h-full before:w-full before:content-[''] bottom-0 data-[side=none]:before:bottom-[-100%]" />
          </UISelect.Popup>
        </UISelect.Positioner>
      </UISelect.Portal>
    </UISelect.Root>

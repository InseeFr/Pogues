...
describe.skip('Transformation entities - Declaration', () => {
  test('Should produce expected STATE in declarations creation from FORM', () => {
    ...
    const state = declarationTransformer.formToComponentState(declarationsFormNew);
  ...
  test('Should produce expected STATE from questionnaire MODEL', () => {
    expect(DeclarationTransformerFactory().modelToState(declarationsModel)).toEqual(declarationsState);
  });
  ...
  test('Should produce expected FORM from questionnaire STATE', () => {
    ...
    expect(declarationTransformer.stateToForm()).toEqual(declarationsFormUpdate);
  });

  test('Should produce expected MODEL from questionnaire STATE', () => {
    const declarationTransformer = DeclarationTransformerFactory({ initialState: declarationsState });

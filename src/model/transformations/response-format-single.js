...
import { UI_BEHAVIOUR, DATATYPE_NAME, DEFAULT_CODES_LIST_SELECTOR_PATH } from 'constants/pogues-constants';

const { TEXT } = DATATYPE_NAME;

export function remoteToState(remote) {
  const {
    responses: [{ Datatype: { visualizationHint: visHint }, mandatory, nonResponseModality, CodeListReference }],
  } = remote;
...
export function stateToRemote(state, collectedVariables) {
  const { [DEFAULT_CODES_LIST_SELECTOR_PATH]: { id: codesListId }, visHint, mandatory } = state;
  return {
    Response: [

 ...
 * question, etc.
 *
 * The 'isDeletedOnReset' attribute indicates whether this external variable
 * must be deleted when Sabiane performs a questionnaire reset. Such a reset
 * may be triggered either manually by the interviewer (for instance when
 * selecting "move" in the survey identification form) or automatically when
 * Sabiane receives a move event from Platine.
 */
export type ExternalVariableType = VariableType & {
  type: VariableTypeType.ExternalVariableType;
  /**
   * Indicates whether the external variable should be deleted when a
   * questionnaire reset is performed by Sabiane.
   *
   * - true  : the value is deleted
   * - false : the value is preserved
   *
   * Default value is false if the attribute is not provided in the JSON.
   *
   * This attribute is used in particular to distinguish:
   *
   * - external variables related to the household, which must be
   * deleted when a move or a reset occurs,
   * - from external variables describing the dwelling, which must be
   * preserved.
   */
...
export type BooleanDatatypeType = DatatypeType & {
  type: 'BooleanDatatypeType';
};

export type DateDatatypeType = DatatypeType & {
...
export type DurationDatatypeType = DatatypeType & {
...
export type NumericDatatypeType = DatatypeType & {
...
export type TextDatatypeType = DatatypeType & {

import { DIMENSION_TYPE, DATATYPE_VIS_HINT } from 'constants/pogues-constants';
...
function measureFromResponse(response) {
  ...
  if (Object.hasOwnProperty.call(response, 'codeListReference')) {
...
export default function parseTable(responses, responseStructure) {
  ...
  // at most two axes
  const [primary, secondary] = dimensions.filter(
    ({ dimensionType }) => dimensionType === PRIMARY || dimensionType === SECONDARY
  );

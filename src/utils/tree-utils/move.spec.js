...
describe('tree utils', () => {
  describe('move question', () => {
    test('should be inserted after if dest is after origin', () => {
      const qAfter = moveComponent(qIni, 'root', 'Q1B1', 'Q1B2');
      expect(qAfter.SSEQ1B.childCmpnts).toEqual(['Q1B2', 'Q1B1']);
    });

    test('should be inserted before if dest is before origin', () => {
  ...
  describe.only('move sequence', () => {
    test('should be move with all its children', () => {
      const qAfter = moveComponent(qIni, 'root', 'SSEQ2B', 'SEQ2');
      expect(qAfter.SEQ1.childCmpnts).toEqual(['Q11', 'SSEQ1A', 'SSEQ1B', 'SSEQ1C', 'SSEQ2B']);
      expect(qAfter.SSEQ2B.childCmpnts).toEqual(['Q2B1', 'Q2B2']);
    });

    test.skip('should break existing sequences to keep the same depth', () => {
      const qAfter = moveComponent(qIni, 'root', 'SSEQ2A', 'Q1B2');
      expect(qAfter.SEQ1.childCmpnts).toEqual(['Q11', 'SSEQ1A', 'SSEQ1B', 'SSEQ2A', 'SSEQ1C']);
      expect(qAfter.SSEQ1B.childCmpnts).toEqual(['Q1B1']);
    });

    test.skip('should adapt if we move before first sequence', () => {
      ...
      expect(qAfter.root.childCmpnts).toEqual(['SSEQ2A', 'SEQ1', 'SEQ2']);
    });
  });
});

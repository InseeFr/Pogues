...
`);
  ...
  describe('removeComponent', () => {
    describe('basic operations', () => {
      ...
      test('should remove the component from the dictionary', () => {
        expect(qAfter.Q1B1).toBeUndefined();
      });

      test('should remove the component from its parent children list', () => {
        expect(qAfter.SSEQ1B.childCmpnts).toEqual(['Q1B2']);
      });
    });

    describe('remove subsequence', () => {
      test('should add its children to the last open sequence', () => {
        const qAfter = removeComponent('root', 'SSEQ1B', qIni);
        expect(qAfter.SSEQ1A.childCmpnts).toEqual(['Q1B1', 'Q1B2']);
      });
    });
    ...
    describe('remove first level sequence', () => {
      test('should add its children to the last open sequence', () => {
        ...
        expect(qAfter.SEQ2).toBeUndefined();

        expect(qAfter.SEQ1.childCmpnts).toEqual(['Q11', 'SSEQ1A', 'SSEQ1B', 'SSEQ1C', 'SSEQ2A']);
      });

      test('should add its children to root if no opened sequence', () => {
        const qAfter = removeComponent('root', 'SEQ1', qIni);

        expect(qAfter.root.childCmpnts).toEqual(['Q11', 'SSEQ1A', 'SSEQ1B', 'SSEQ1C', 'SEQ2']);
      });
    });
  });
});

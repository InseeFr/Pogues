...
function transformationStateToFormPrimary(currentState, codesListsStore, codesListsTransformers) {
  ...
  codesListsTransformers[PRIMARY] = { [CODES_LIST]: codesListTransformer };
...
function transformationFormToStatePrimary(form, codesListsTransformers) {
  ...
  if (typePrimary === CODES_LIST) {
    ...
    const codesListState = codesListsTransformers[PRIMARY][CODES_LIST].formToState(codesListForm);
    state[CODES_LIST].codesListId = codesListState.id;
  } else {
...
function transformationFormToStateSecondary(form, codesListsTransformers) {
  const { showSecondaryAxis, showTotalLabel, totalLabel, [NEW]: codesListForm } = form;
  const state = {
    showSecondaryAxis,
    showTotalLabel,
    totalLabel,
  ...
  if (showSecondaryAxis) {
    const codesListState = codesListsTransformers[SECONDARY].formToState(codesListForm);
    state[SECONDARY].codesListId = codesListState.id;
  }
...
function transformationFormToStateMeasure(form, codesListsTransformers) {
  ...
  if (type === SIMPLE) {
    ...
    const codesListState = codesListsTransformers[MEASURE].formToState(codesListForm);
    state[SINGLE_CHOICE] = {
      visHint,
...
function transformationFormToStateListMeasure(form, codesListsTransformers) {
  const { measures } = form;
  // @TODO
  return measures.map(measure => transformationFormToStateMeasure(measure, codesListsTransformers[LIST_MEASURE]))
}
...
function transformationFormToState(form, codesListsTransformers) {
  ...
  return {
    [PRIMARY]: transformationFormToStatePrimary(primaryForm, codesListsTransformers[PRIMARY]),
    [SECONDARY]: transformationFormToStateSecondary(secondaryForm, codesListsTransformers[SECONDARY]),
    [MEASURE]: transformationFormToStateMeasure(measureForm, codesListsTransformers[MEASURE]),
    [LIST_MEASURE]: transformationFormToStateListMeasure(listMeasureForm, codesListsTransformers[LIST_MEASURE]),
  };
}

function transformationStateToFormSecondary(currentState, codesListsStore, codesListsTransformers) {
  const state = {
  ...
  codesListsTransformers[SECONDARY] = codesListTransformer;
...
function transformationStateToFormMeasure(currentState, codesListsStore, codesListsTransformers, isList = false) {
  const state = {
  ...
  if (isList) {
    codesListsTransformers.push(codesListsTransformers);
  } else {
    codesListsTransformers[MEASURE] = codesListTransformer;
  }
...
function transformationStateToFormListMeasure(currentState, codesListsStore, codesListsTransformers) {
  codesListsTransformers[LIST_MEASURE] = {};
  // @TODO
  return {
    ...defaultListMeasuresForm,
    measures: currentState.map(measureState =>
      transformationStateToFormMeasure(measureState, codesListsStore, codesListsTransformers[LIST_MEASURE], true)
    ),
  };
}

function transformationStateToForm(currentState, codesListsStore, codesListsTransformers) {
  codesListsTransformers[TABLE] = {};
  const {
    [PRIMARY]: primaryState,
    [SECONDARY]: secondaryState,
    [MEASURE]: measureState,
  ...
  return {
    [PRIMARY]: transformationStateToFormPrimary(primaryState, codesListsStore, codesListsTransformers[TABLE]),
    [SECONDARY]: transformationStateToFormSecondary(secondaryState, codesListsStore, codesListsTransformers[TABLE]),
    [MEASURE]: transformationStateToFormMeasure(measureState, codesListsStore, codesListsTransformers[TABLE]),
    [LIST_MEASURE]: transformationStateToFormListMeasure(
      listMeasureState,
      codesListsStore,
      codesListsTransformers[TABLE]
    ),

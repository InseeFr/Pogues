...
describe('remove', () => {
  [SEQUENCE, SUBSEQUENCE, QUESTION].forEach(type => {
    test(`For a ${type} without children, should call removeLeafComponent`, () => {
      const result = component.remove({ '1': { type, children: [] } }, '1');
      expect(result).toEqual({
        removeLeafComponent: true,
        activesComponents: { '1': { type, children: [] } },
        deletedComponent: { type, children: [] },
      });
    });
  });
  test(`For a SEQUENCE with children, should call removeSequence`, () => {
    const result = component.remove({ '1': { type: SEQUENCE, children: ['2'] } }, '1');
    expect(result).toEqual({
      removeSequence: true,
      activesComponents: { '1': { type: SEQUENCE, children: ['2'] } },
      deletedComponent: { type: SEQUENCE, children: ['2'] },
    });
  });
  test(`For a SUBSEQUENCE with children, should call removeSubSequence`, () => {
    const result = component.remove({ '1': { type: SUBSEQUENCE, children: ['2'] } }, '1');
    expect(result).toEqual({
      removeSubSequence: true,
      activesComponents: { '1': { type: SUBSEQUENCE, children: ['2'] } },
      deletedComponent: { type: SUBSEQUENCE, children: ['2'] },
    });
  });
});

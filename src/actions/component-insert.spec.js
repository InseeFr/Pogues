...
describe('moveQuestionToSubSequence', () => {
  ...
  test(`should return an object with the move of a question to the new subsequence`, () => {
    ...
    const result = component.moveQuestionToSubSequence(activeComponents, components[1], newSubSequence);

    expect(result['1']).toEqual({ id: '1', children: ['2'], type: SEQUENCE });
    expect(result['3']).toEqual({ id: '3', weight: 0, parent: '4', type: QUESTION });
    expect(result['4']).toEqual({ id: '4', weight: 1, parent: '1', children: ['3'], type: SUBSEQUENCE });
  });
...
describe('moveQuestionAndSubSequenceToSequence', () => {
  ...
  test('should return all changes need', () => {
    ...
    expect(result['3']).toEqual({ id: '3', weight: 0, type: QUESTION, parent: '9', children: [] });

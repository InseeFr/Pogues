import { REMOVE_CODE_LIST, LOAD_CODE_LIST_SUCCESS, EDIT_CODE_LIST } from 'actions/code-list';

import { NEW_CODE_LIST_FORMAT } from 'actions/response-format';
import { CREATE_CODE, REMOVE_CODE, MOVE_UP_CODE, MOVE_DOWN_CODE } from 'actions/code';
import { LOAD_CLIST_SPECS_SUCCESS } from 'actions/code-list-specification';
...
function moveUpCode(codes, id) {
  ...
  if (index > 0) return [...codes.slice(0, index - 1), codes[index], codes[index - 1], ...codes.slice(index + 1)];
  return codes;
}
...
function moveDownCode(codes, id) {
  ...
  if (index < codes.length - 1)
    return [...codes.slice(0, index), codes[index + 1], codes[index], ...codes.slice(index + 2)];
  return codes;
}
